...
class LCD
{
  ...
  ///
  /// The data is stored in RGB format, which is packed as 0x00RRGGBB. This leaves us with 2 bytes of unused data in at the front, which we use to hold current pixel priorities.
  ///
  ...
  const BufferedImage screenBuffer = new BufferedImage(W, H, BufferedImage.TYPE_INT_RGB);

  /// Background palettes. On CGB, 0-7 are used. On GB, only 0 is used.
  const IPalette[] bgPalettes = new IPalette[8];

  /// Sprite palettes. 0-7 used on CGB, 0-1 used on GB.
  const IPalette[] spritePalettes = new IPalette[8];

  /// Background palette memory on the CGB, indexed through $FF69.
  const byte[] gbcBackgroundPaletteMemory = new byte[0x40];

  /// Sprite palette memory on the CGB, indexed through $FF6B.
  const byte[] gbcSpritePaletteMemory = new byte[0x40];
  ...
  const int[] spritesDrawnPerLine = new int[144];
  ...
  void loadPalettesFromMemory(byte[] from, IPalette[] to)
  {
    // 8 palettes
    for (int i = 0; i < 8; i++)
    {
  ...
  void updatePaletteByte(byte[] from, IPalette to, int i, int j)
  {
  ...
  void drawBackgroundTiles(int[] data, int scanline)
  {
  ...
  void drawWindow(int[] data, int scanline)
  {
  // Local reference to save time
  byte[] vram = core.mmu.vram;
  ...
  void drawTile(IPalette palette, int[] data, int x, int y, int tile, int scanline,
  ...
  void drawSprites(int[] data, int scanline)
  {
